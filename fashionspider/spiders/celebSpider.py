...
class CelebSpider(scrapy.Spider):
    ...
    def subsectionParse(self, response):
        ...
        imgTail = spider_init.siteRules.get(site)["imgTail"]
        ...
        for index,subsectionImg in enumerate(response.css(subsectionImgRule).getall()):
            ...
            if (isSrcSet):
                ...
                imgUrl = imgRelativeTo + imgUrl if imgUrl else '' + imgTail
                self.data[site]["sections"][section]["subsections"].append(imgUrl)
            else:
